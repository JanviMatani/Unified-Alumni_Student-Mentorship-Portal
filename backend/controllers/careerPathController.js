...
exports.getCareerPaths = async (req, res) => {
    ...
    try {
        ...
        const fallback = [
            {
                ...
                difficulty: "Easy",
                stages: [
                    { title: "Intern", year: "0-6 Months", skills: [{ name: "Javascript", weight: 8 }], salary: "15-20k/mo" },
                    { title: "Junior Dev", year: "6-12 Months", skills: [{ name: "React/Node", weight: 9 }], salary: "6-9 LPA" }
                ],
                creatorId: null
            },
            {
                ...
                difficulty: "Medium",
                stages: [
                    { title: "Trainee", year: "0-3 Months", skills: [{ name: "Java", weight: 7 }], salary: "5 LPA" },
                    { title: "SDE-1", year: "3-12 Months", skills: [{ name: "DSA", weight: 9 }], salary: "12 LPA" }
                ],
                creatorId: null
            },
            {
                ...
                difficulty: "Hard",
                stages: [
                    { title: "SDE-2", year: "Year 2", skills: [{ name: "System Design", weight: 9 }], salary: "18-24 LPA" },
                    { title: "Tech Lead", year: "Year 4", skills: [{ name: "Architecture", weight: 10 }], salary: "30+ LPA" }
                ],
                creatorId: null
            }
        ];
        ...
        // If company type specified but no match, return empty to avoid showing wrong type
        if (companyType && filteredFallback.length === 0) {
            return res.json([]);
        }

        res.json(filteredFallback.length ? filteredFallback : fallback);
        ...
        res.json([{
            ...
            entryLevel: "Beginner",
            companyType: "Startup",
            stages: [{ title: "System Online", year: "Now", skills: [] }]
        }]);
    }
};
...
// @access  Private
exports.getMyPaths = async (req, res) => {
    try {
